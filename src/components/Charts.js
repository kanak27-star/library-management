...
const Charts = () => {
  ...
  useEffect(() => {
    if (selectedYear) fetchMonthlyData(selectedYear);
  }, [selectedYear]);

  // Fetch annual data
  const fetchAnnualData = async () => {
    try {
      const response = await fetch(`${API_URL}/api/counts/annual`);
      const data = await response.json();

      // Fill missing years from 2020 to 2025 with 0
      const annualCounts = {};
      ...
      // Set default selected year if current year not in data
      if (!filledData.find(d => d._id === selectedYear)) {
        setSelectedYear(filledData[0]._id);
      }
    } catch (error) {
  ...
  const fetchMonthlyData = async (year) => {
    try {
      const response = await fetch(`${API_URL}/api/counts/monthly/${year}`);
      const data = await response.json();
  ...
  const options = {
    ...
    plugins: {
      legend: { position: 'top' },
      title: { display: true, text: 'Book Borrowing Statistics' },
    },
  };
    ...
    <div className="p-4">
      ...
      <div>
        <h3 className="text-xl font-semibold mb-2">
          Monthly Book Borrowings for {selectedYear}
        </h3>
        <select
          value={selectedYear}
