  ...
  return (
    <div className="App">
      <Header
        ...
        onChartsClick={() => handleButtonClick("charts")}
      ...
      >
        <div className="bg-gray-800 rounded-lg shadow-gray-700 justify-center shadow-2xl max-w-fit">
          {activeForm === "addBook" && <AddBook onBookAdded={() => { setRefreshKey(prev => prev + 1); setActiveForm("goToHome"); }} />}
          {activeForm === "addBorrower" && <AddBorrower onBorrowerAdded={() => { setRefreshKey(prev => prev + 1); setActiveForm("goToHome"); }} />}
          {activeForm === "addAuthor" && <AddAuthor onAuthorAdded={() => { setRefreshKey(prev => prev + 1); setActiveForm("goToHome"); }} />}
          {activeForm === "updateBook" && <UpdateBook />}
          {activeForm === "updateAuthor" && <UpdateAuthor />}
          {activeForm === "updateBorrower" && <UpdateBorrower />}
          {activeForm === "deleteBorrower" && <DeleteBorrower onBorrowerDeleted={() => { setRefreshKey(prev => prev + 1); setActiveForm("goToHome"); }} />}
          {activeForm === "deleteBook" && <DeleteBook onBookDeleted={() => { setRefreshKey(prev => prev + 1); setActiveForm("goToHome"); }} />}
